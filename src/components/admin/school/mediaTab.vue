<template>
    <div class="mb-3">
      ...
      <div class="media-gallery grid">
        ...
        <div v-if="galleryItems.length ==0"> <p>No Media</p></div>
      </div>
  ...
  const emit = defineEmits(['updateMedia']); // Define the event
  ...
  const removeMediaItem = async (media_id, event) => {
    ...
    try {
      const response = await $adminService.school_media_delete(media_id);
  ...
  const uploadMediaFiles = async () => {
    ...
    try {
      const response = await $adminService.upload_school_media(formData);
      if (response.status === 200) {
