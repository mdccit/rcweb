<template>
    <div v-if="connectShow" class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-8"> This school has been matched
        up to a
        government school. You can now synchronize the data from their source.<br><br> Gov ID:
        <strong>{{ props.govId }}</strong><br><br><br> Wish to disconnect?
        <form method="POST"
            action="/">
    ...
    <div v-if="connectShow" class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-8">
        ...
        <fieldset>
            <div class="grid grid-cols-4 gap-4">
                <div v-for="setting in settings"><label class="flex items-center"><input name="setting.name"
                            @change="changeSetting(setting)" type="checkbox"
                            ...
                            :value="setting.value" :true-value="true" :checked="setting.value"><span class="ml-4">{{
                            setting.label }}</span></label></div>
