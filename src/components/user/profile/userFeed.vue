            ...
            <div class="basis-full flex flex-col  ">
              ...
              <div class="flex justify-end mt-2">
                ...
                <button v-if="editingPostId == post.id" @click="startEditPost(post.id)"
                  class="bg-steelBlue hover:bg-darkAzureBlue transition text-white px-8 py-2 rounded-lg text-sm min-w-24" :disabled="postEdit">
                    ...
                    <span v-if="!postEdit">Update</span>
                </button>
              </div>
            </div>
          </div>
...
<script setup>
import { defineProps, defineEmits, defineExpose, ref, onMounted, watch, nextTick } from 'vue';
...
const loadInitialPosts = async () => {
  try {
    ...
    loading.value = true; // Ensure loading is set to true

    // Simulate a delay to ensure loader shows
    await new Promise(resolve => setTimeout(resolve, 2000)); // Simulate 2s delay for testing

    // Emit event or call API to load posts
    await emit('listpost');
  } catch (error) {
    console.error('Error loading initial posts:', error);
  } finally {
    loading.value = false; // Hide the loader after posts are fetched
    console.log("Initial posts loaded, loader hidden");
  }
};
