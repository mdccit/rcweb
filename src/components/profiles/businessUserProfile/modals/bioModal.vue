...
onMounted(() => {
    if (props.slug) {
        ...
        fetchBusinessUserBio(props.slug);
    }
});
...
const fetchBusinessUserBio = async (slug) => {
    try {
        console.log(slug);
...
// Function to update business User bio
const updateBusinessUserBio = async (bio) => {
    try {
        const request_body = { user_slug: props.slug, bio: bio };  // Construct request body with bio
        const response = await $userService.update_business_user_bio(request_body);  // Pass slug and request body
...
const saveBio = () => {
    updateBusinessUserBio(user_bio.value);
}
