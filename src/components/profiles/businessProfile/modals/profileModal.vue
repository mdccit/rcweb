<template>
        ...
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
          <div
                ...
                <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left w-full">
                  ...
                  <div class="mt-2 w-full">
                    ...
                    <div class="w-full flex justify-center">
                      <div v-if="props.data.profile !=null">
                        <img class="mx-auto w-44 h-44 rounded-[30px] mt-3" :src="props.data.profile.url" alt="">
                        <!-- <button @click="removeProfile">Remove</button> -->
                        <div class="w-full flex justify-center">
  
                        </div>
                        <button class="p-2 rounded-lg bg-red-600 mx-auto m-2 text-white" @click="removeprofile"><svg
            ...
            <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
              ...
              <button type="button" @click="$emit('close', 'profile')"
  ...
  const saveProfilePicture = async () => {
    ...
    try {
      loading.value = true;
      const response = await $publicService.upload_business_profile_picture(profile_picture.value, props.slug); // Call the upload function
  ...
  const removeprofile = async () => {
    try {
        const data ={
            ...
            media_id:props.data.profile.media_id
        }
      const dataSets = await $publicService.update_business_media_remove(data);
      emit('close', 'profile');
    } catch (error) {
