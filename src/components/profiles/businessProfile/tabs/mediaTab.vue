<template>
      ...
      <form @submit.prevent="uploadMedia" class="upload-form" >
        ...
        <div class="upload-btn-wrapper mt-4">
          <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-700 w-full text-center"
            ...
            Upload Media
            <svg v-if="loading" aria-hidden="true" class="inline w-4 h-4 me-3 text-white animate-spin"
  ...
  const uploadMedia = async () => {
    ...
    formData.append('business_slug', props.userSlug);
    ...
    try {
      ...
      const response = await $publicService.upload_business_media(formData);
  ...
  const fetchGalleryBySlug = async () => {
    galleryItems.value= props.galleryItems
    // try {
