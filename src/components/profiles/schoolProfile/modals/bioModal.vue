<template>
    ...
    <div v-if="visible" class="relative z-index-400" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        ...
        <form @submit.prevent="saveSchoolBio">
                                        ...
                                        <div class="w-full">
                                            <div class="w-full">
                                                ...
                                                <div class="flex rounded-lg border border-gray-300 shadow-sm">
                                                    <textarea id="school_bio" v-model="school_bio"
                                                        autocomplete="school_bio"
...
// Watch for changes in the props and log the new values
watch(
    () => props.visible,
    (newVal) => {
        if (newVal) {
            school_bio.value = props.schoolData.bio ?? "N/A";
        }
    }
);
...
const updateSchoolBio = async () => {
    ...
    try {
        console.log(school_bio.value);
        const request_body = { user_slug: props.slug, bio: school_bio.value };  // Construct request body with bio
        const response = await $publicService.update_school_bio(request_body);  // Pass slug and request body
...
const saveSchoolBio = () => {
    updateSchoolBio(school_bio.value);
}
