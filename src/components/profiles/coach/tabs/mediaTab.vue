<template>
  <div>
    ...
    <div class="media-gallery grid">
      <div v-for="(item, index) in galleryItems" :key="index" data-fancybox="gallery" class="media-item"  :src="item.src">
        ...
        <button @click="removeMediaItem(item.media_id, $event)" class="bg-red-500 text-white px-2 py-1 mt-2 rounded hover:bg-red-700">
          Remove
        </button>
      </div>
    </div>
...
// Sync local galleryItems with props.galleryItems when prop changes
watch(
  () => props.galleryItems,
  (newItems) => {
...
const removeMediaItem = async (media_id, event) => {
  ...
  try {
    ...
    const response = await $userService.delete_player_media(media_id);
...
const uploadMedia = async () => {
  ...
  try {
    ...
    if (response.status == 200) {
      ...
      // Emit the event after successful upload
      emit('uploadMedia');
    } else {
      loading.value = false;
      nuxtApp.$notification.triggerNotification(response.display_message, 'warning');
  ...
  galleryItems.value = mediaInfo.media_urls.map(media => {
    if (media.media_type === 'image') {
      ...
      return {
        type: 'video',
        href: media.url,
        src: media.url || 'https://via.placeholder.com/200x150.png?text=Video',
        media_id: media.media_id,
      };
    }
  });
