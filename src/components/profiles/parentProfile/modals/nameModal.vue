                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:text-left w-full">
                                ...
                                <div class="mt-2 w-full">
                                    ...
                                    <div class="w-full mt-3">
                                        ...
                                        <div  v-if="props.data.profile_picture !=null">
                                            <img class="mx-auto w-44 h-44 rounded-[30px] mt-3" :src="props.data.profile_picture.url" alt="">
                                            <!-- <button @click="removeProfile">Remove</button> -->
                                        </div>
                                        <div  v-if="props.data.profile_picture !=null" class="w-full flex justify-center">
...
watch(() => props.visible, () => {
    ...
    other_names.value =  props.data.other_name;
});
...
const saveProfilePicture = async () => {
    ...
    try {
        ...
        const response = await $publicService.upload_parent_profile_photo(profile_picture.value, user_slug); // Call the upload function
...
const updatePlayerNames = async (firstName, lastName, otherNames) => {
    ...
    try {
        ...
        const response = await $publicService.update_parent_basic_info(request_body); // Call the API to update the names
        if (response.status == '200') {
            loading.value = false;
            nuxtApp.$notification.triggerNotification(response.display_message, 'success');
...
const removeProfile =async() =>{
    try {
        const dataSets = await $publicService.delete_media_parent(props.data.profile_picture.media_id);
        emit('dataUpdate')
    } catch (error) {
