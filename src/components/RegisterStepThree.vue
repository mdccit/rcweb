<template>
  <div class="min-h-screen w-full bg-generic bg-cover bg-no-repeat flex flex-col sm:justify-center items-center py-12 px-4">
    <div class="w-full mt-6 mx-4 p-12 bg-white rounded-lg overflow-hidden sm:max-w-3xl">
      ...
      <form @submit.prevent="handleSubmitStep3">
        <div class="flex justify-center space-x-4">
          <div v-for="pkg in packages" :key="pkg.value" class="border w-[230px] rounded-lg text-center p-3 relative cursor-pointer" @click="selectedPackage = pkg.value">
            <input class="radio-input absolute h-24 m-0 cursor-pointer z-2 opacity-0 peer" :id="pkg.value" type="radio" :value="pkg.value" v-model="selectedPackage" name="pkg" />
        ...
        <div class="flex items-center justify-end mt-6">
          <button type="submit" class="border rounded-full shadow-sm py-2 px-4 focus:outline-none focus:ring focus:ring-opacity-50 bg-steelBlue hover:bg-darkAzureBlue text-white border-transparent focus:border-lightAzure focus:ring-lightPastalBlue ml-4 !px-8 !py-2.5 transition">
    ...
    <PaymentComponent v-if="selectedPackage === 'premium'" :stripeCustomerId="stripeCustomerId" :handleSubscription="createSetupIntent" :confirmSetupIntent="confirmSetupIntent" :finalizeSubscription="finalizeSubscription" />
  </div>
</template>
  ...
  if (!selectedPackage.value) {
    ...
    if (selectedPackage.value === 'premium') {
      try {
        ...
        console.log('Stripe customer ID:', stripeCustomerId.value);
        // Now you can proceed with the payment process using stripeCustomerId.value
        // Example: Call Stripe setup intent API, handle payment, etc.
